...
1. **生成密钥对**（[步骤一](#31-步骤一启动认证命令)）：客户端生成一个随机字符串 `code_verifier`，然后计算其 SHA256 哈希值作为 `code_challenge`
   ```
...
2. **发送 code_challenge**（[步骤一](#31-步骤一启动认证命令)）：在授权请求中只发送 `code_challenge`，不发送 `code_verifier`

3. **验证身份**（[步骤四](#34-步骤四token-安全交换---pkce-的关键作用)）：Token 交换时提供 `code_verifier`，服务器验证：
   ```
...
**State 参数如何防护**：
1. 客户端生成一个随机的 `state` 值（[步骤一](#31-步骤一启动认证命令)）
2. 在授权请求中发送这个 `state` 值
3. 授权服务器在回调时原样返回 `state` 值（[步骤三](#33-步骤三回调页面与授权码获取)）
...
| **安全机制** | `state` | `1ejObPePvfbUNBtWwaWYr6vm8AUHMCQJbo6tsfKjKiU` | 防止 CSRF 攻击（[详见预备知识](#23-state-参数与-csrf-防护)） |
| **安全机制** | `code_challenge` | `aM_o8LfwOVdvgSNkK3Gr4RLWS4olNGv4tuGBl3X3_Mo` | PKCE 挑战码（[详见预备知识](#22-pkce-安全机制)） |
...
1. **浏览器重定向**：用户确认授权后，Anthropic 将浏览器重定向到[步骤一](#31-步骤一启动认证命令)中指定的 `redirect_uri`：
   ```
...
3. **状态验证**：`state` 参数原样返回，Claude Code 会验证其是否与步骤一发送的一致（[详见预备知识](#23-state-参数与-csrf-防护)）

这一步就像 Anthropic 给了你一张"取货凭证"（授权码）。这张凭证有效期很短，必须赶紧拿去换取真正的"通行证"（访问令牌）。

### 3.4 步骤四：Token 安全交换 - PKCE 的关键作用
...
此时 Anthropic 服务器会执行 PKCE 验证（[详见预备知识](#22-pkce-安全机制)）：
